...
final readonly class ElasticApmInteractor implements ElasticApmInteractorInterface
{
    ...
    public function beginTransaction(
        ...
        if (\version_compare(ElasticApm::VERSION, '1.3.0', '<')) {
            return ElasticApm::beginTransaction(
                $name,
                $type,
                $timestamp,
                $distributedTracingData ? $distributedTracingData->serializeToString() : null,
            );
        }
    ...
    public function beginCurrentTransaction(
        ...
        if (\version_compare(ElasticApm::VERSION, '1.3.0', '<')) {
            return ElasticApm::beginCurrentTransaction(
                $name,
                $type,
                $timestamp,
                $distributedTracingData ? $distributedTracingData->serializeToString() : null,
    ...
    private function createTransactionBuilder(
        string $name,
        string $type,
        ?float $timestamp = null,
        ?DistributedTracingData $distributedTracingData = null,
